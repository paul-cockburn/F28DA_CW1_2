 ...
 * <ol>
 * <li>Create an object of the class FileWordRead by providing a file name: <br>
 * {@code FileWordRead readWords = new FileWordRead("filename.txt");}</li>
...
public class FileWordReader {
	...
	private FileWordReader() {
	}

	/**
	 * Creates a new {@code FileWordReader} object to read words from a given file
	 * (the file name is given as argument).
	 * 
	 * @param inputFileName
	 *            The name of the file to read words from
	 */
	public FileWordReader(String inputFileName) throws IOException {
	 ...
	 */
	public FileWordReader(File inputFile) throws IOException {
	 ...
	 * line number.
	 */
	private WLoc readWord() throws IOException {
		int nextByte;
		char nextChar;
		...
		while (true) {
			...
			if (nextByte == -1) {
				...
				if (buffer.length() == 0) {
					...
					return (new WLoc(fileName, wordLine, buffer.toString()));
				}
			}
	...
	public WLoc nextWord() throws java.io.IOException {
		WLoc toReturn = nextWord;
		if (!endOfFile) {
			nextWord = readWord();
		} else {
			nextWord = null;
